<?php	
session_start();
   //AUTOLOAD
function autocarga($clase){

    $ruta = "./controladores/$clase.php"; 
    if (file_exists($ruta)){ 
      include_once $ruta; 
    }

    $ruta = "./modelos/$clase.php"; 
    if (file_exists($ruta)){ 
        include_once $ruta; 
    }

    $ruta = "./vistas/$clase.php"; 
    if (file_exists($ruta)){ 
        include_once $ruta; 
    }
} 
spl_autoload_register("autocarga");


  //Función para filtrar los campos del formulario
  function filtrado($datos){
    $datos = trim($datos); // Elimina espacios antes y después de los datos
    $datos = stripslashes($datos); // Elimina backslashes \
    $datos = htmlspecialchars($datos); // Traduce caracteres especiales en entidades HTML
    return $datos;
}

if ($_REQUEST) {
    
        if (isset($_REQUEST['accion'])) {

                // INICIO
                if ($_REQUEST['accion'] == "inicio") {
                    ControladorLogin::mostrarFormularioLogin();
                }

                //  INICIO - LOGIN FAIL
                if ($_REQUEST['accion'] == "error") {
                    ControladorLogin::mostrarFormularioLoginError();
                }

                //  COMPRUBA LOGIN EN BBDD
                if ($_REQUEST['accion'] == "checkLogin") {
                    $email = filtrado($_REQUEST['email']);
                    $password = filtrado($_REQUEST['password']);
                    ControladorLogin::chequearLogin($email, $password);
                }

                // MOSTRAR CANCIONES
                if ($_REQUEST['accion'] == "mostrar") {
                    ControladorCanciones::mostrarCanciones();
                }

                // MOSTRAR CANCIONES TOP
                if ($_REQUEST['accion'] == "mostrarTop") {
                    ControladorCanciones::mostrarCancionesTop();
                }

                // MODIFICAR CANCIONES
                if ($_REQUEST['accion'] == "modificarCancion") {

                    $idCancion = filtrado($_REQUEST['idCancion']);
                    $puntuacion = filtrado($_REQUEST['numero']);

                    // $idCancion = filtrado($_REQUEST['idCancion']);
                    
                    // if($_REQUEST['numero']==null){
                    //     $puntuacion=0;
                    // }else{
                    //     $puntuacion = filtrado($_REQUEST['numero']);
                    // }
                    
                    ControladorCanciones::modificarCancion($idCancion,$puntuacion);
                }

                // MODIFICAR CANCIONES TOP
                if ($_REQUEST['accion'] == "modificarCancionTop") {

                    $idCancion = filtrado($_REQUEST['idCancion']);
                    $puntuacion = filtrado($_REQUEST['numero']);
                    ControladorCanciones::modificarCancionTop($idCancion,$puntuacion);

                //-------------------------------------------------------------------

            }  
        }        
    }
?>